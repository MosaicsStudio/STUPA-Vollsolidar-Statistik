...
class Correlation:
    ...
    def scatter_with_category(self, df: DataFrame, fig: Figure, category: 'Question', x_log: bool=False, y_log: bool=False, colors: List[str]=[], **kwargs: Any) -> None:
        if self.question_a.type != QuestionType.NUMBER or self.question_b.type != QuestionType.NUMBER:
            raise ValueError("Both questions must be of type 'number' for a scatter plot")
        ...
        df[self.question_a.code] = pd.to_numeric(df[self.question_a.code], errors='coerce')
        df[self.question_b.code] = pd.to_numeric(df[self.question_b.code], errors='coerce')
        ...
        for i, (key, group) in enumerate(category.group_frame(df)):
            ...
            # Add a linear regression line using numpy
            x = group[self.question_a.code]
            y = group[self.question_b.code]

            # Sort x-values to have a continuous line
            x, y = zip(*sorted(zip(x, y)))

            m, b = np.polyfit(x, y, 1)

            ax.plot(x, m*x + b, color=colors[i] if i < len(colors) else None)
